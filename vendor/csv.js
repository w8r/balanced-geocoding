...
// Mr-Data-Converter
//  
// Input CSV or Tab-delimited data and this will parse it into a Data Grid
// Javascript object
//  
...
var CSVParser = {
    ...
    // PARSE
    // ---------------------------------------
    // var parseOutput = CSVParser.parse(this.inputText,
    // this.headersProvided, this.delimiter, this.downcaseHeaders,
    // this.upcaseHeaders);

    parse : function(input, headersIncluded, delimiterType, downcaseHeaders,
            upcaseHeaders) {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            if (numValues != numColumns) {
                this.log("Error parsing row " + String(i)
...
var CSV = function(headers, opts) {
    ...
    var specialChars =
            new RegExp('['
                            + [options.quoteChar, options.colSep,
                                    options.rowSep].join('') + ']', 'i');
    ...
    return {
         ...
         * adds a new row
         * 
         * when the first argument is an array, it is used as the row data,
         * otherwise all arguments are used
         */
        push : function() {
            var data;
